...
console.log('Loading AuthHandler function');
...
var config = require('./src/config/' + evnResolved + '-config.json');
var RestClient = require('./src/services/rest/restClient');
console.log('config papiHost:', config.papiHost);
var restClient = new RestClient(config.papiHost, config.papiPort, config.deviceId, config.platform);
/**
...
function userApiSuccessCallBack(user, policy, callback) {
    //CountMetric.recordCount("AuthSuccessCallBack")
    ...
    /*authResponse.context = {
        userName: 'Vijay', // $context.authorizer.key -> value
        userId: 112345,
        locale: 'en_US',
    };*/
...
function userApiFailureCallBack(error, policy, callback) {
...
exports.handler = (event, context, callback) => {
    ...
    var UserRestClient = require('./src/services/rest/userRestClient');
    var UserService = require('./src/services/userService');
    var userRestClient = new UserRestClient(restClient);
    var userService = new UserService(userRestClient);
    ...
    var principalId = 'user|vvangapandu';
    ...
    // the example policy below allow/deny access to all resources in the RestApi
    ...
    userService.getUser(bearerToken, function(user) {
        ...
        userApiFailureCallBack(error, policy, callback);
    });

    /*if(event.authorizationToken && authServResponse) {
        policy.allowAllMethods();
        CountMetric.recordCount("AuthSuccess");
        console.log(authServResponse);
